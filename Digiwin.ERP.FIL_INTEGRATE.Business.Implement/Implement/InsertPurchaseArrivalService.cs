...
//<CreateDate>2016/11/16 16:32:58</CreateDate>
//<IssueNO>P001-161101003</IssueNO>
//<Description>产生到货单服务</Description>
//----------------------------------------------------------------  
//20161206 modi by wangyq for P001-161101003
...
namespace Digiwin.ERP.FIL_INTEGRATE.Business.Implement {
    ...
    /// 产生到货单服务 
    /// </summary>
    [ServiceClass(typeof(IInsertPurchaseArrivalService))]
    [Description("产生到货单服务")]
    sealed class InsertPurchaseArrivalService : ServiceComponent, IInsertPurchaseArrivalService {
        ...
        public DependencyObjectCollection InsertPurchaseArrival(string employee_no, string scan_type, DateTime report_datetime, string picking_department_no,
            ...
            //if (string.IsNullOrEmpty(scan_doc_no)) {
            //    throw new BusinessRuleException(string.Format(infoContainer.GetMessage("A111201"), "scan_doc_no"));
            //}
            string category = string.Empty;
            if (recommended_operations == "1") {
                category = "36";
            } else if (recommended_operations == "3") {
                category = "37";
            }
            //20170619 add by zhangcn for P001-170606002 ===begin===
            string infoLotNo = string.Empty;
            if (scanColl != null && scanColl.Count > 0){
                infoLotNo = scanColl[0]["info_lot_no"].ToStringExtension();
            }
            //20170619 add by zhangcn for P001-170606002 ===begin===
            ...
            using (IConnectionService connectionService = this.GetService<IConnectionService>()) {
                ...
                using (ITransactionService transActionService = this.GetService<ITransactionService>()) {
                    InsertBC_LINE(recommended_operations);  //20170406 modi by wangrm for P001-170316001 增加入参//20161208 add by shenbao for B001-161208020 要先新增BC_LINE，确保保存自动审核的单据能正确修改到BC_LINE
                    //InsertPA(report_datetime, category, resultColl);//20170209 mark by liwei1 for P001-170203001
                    InsertPA(report_datetime, category, resultColl, docId, infoLotNo);//20170209 add by liwei1 for P001-170203001
        ...
        //private void InsertPA(DateTime report_datetime, string category, DependencyObjectCollection resultColl) {//20170209 mark by liwei1 for P001-170203001
            //DataTable dt = QueryForPA(report_datetime, category);//20170209 mark by liwei1 for P001-170203001
        private void InsertPA(DateTime report_datetime, string category, DependencyObjectCollection resultColl, object docId, string infoLotNo) {//20170209 add by liwei1 for P001-170203001
            DataTable dt = QueryForPA(report_datetime, category, docId);//20170209 add by liwei1 for P001-170203001
            ...
            if (dt.Rows.Count > 0) {
                ...
                foreach (DataRow dr in dt.Rows) {
                    #region 计算列 20170619 add by zhangcn for P001-170606002
                    DependencyObjectCollection collSupplySyneryFiD = QuerySupplySyneryFiD(infoLotNo);
            ...
            switch(dr_d["temp_INSPECT_MODE"].ToStringExtension()){
                ...
                case "3":
                    if(dr["temp_QC_RESULT_INPUT_TYPE"].ToStringExtension() == "1"){
                        ...
                        //20161206 add by wangyq for P001-161101003  =============begin==============
                        dr_d["INSPECTION_STATUS"] = "2";
                    }
                    //20161206 add by wangyq for P001-161101003  =============end==============
                    break;
            }
        ...
        private void InsertBC_LINE(string recommended_operations) {//20170406 modi by wangrm for P001-170316001 增加入参
            ...
            if (bcLineManagement && recommended_operations=="3") {//20170406 modi by wangrm for P001-170316001 OLD:if (bcLineManagement) {
        ...
        /// <returns></returns>
        //private DataTable QueryForPA(DateTime report_datetime, string category) {//20170209 mark by liwei1 for P001-170203001
        private DataTable QueryForPA(DateTime report_datetime, string category,object docId) {//20170209 add by liwei1 for P001-170203001
            QueryNode node = OOQL.Select(true, OOQL.CreateProperty("Table_scan.ID", "PURCHASE_ARRIVAL_ID"),
                                ...
                                //.On(OOQL.CreateProperty("PARA_DOC_FIL.Owner_Org.ROid") == OOQL.CreateProperty("SUPPLY_CENTER.SUPPLY_CENTER_ID")
                                ...
                                .On(OOQL.CreateProperty("Table_scan.info_lot_no") == OOQL.CreateProperty("PURCHASE_ORDER.DOC_NO"))
        ...
        private DataTable QueryForPA_D(string category, DateTime report_datetime) {
            ...
            QueryNode node = OOQL.Select(true, OOQL.CreateProperty("Table_scan_detail.ID", "PURCHASE_ARRIVAL_D_ID"),
                ...
                //Formulas.RowNumber("SequenceNumber", new OverClause(partitionBy, orderList)),
                ...
                //20161208 add by wangyq for B001-161207009  ============begin==============
                ...
                //20161208 add by wangyq for B001-161207009  ============end==============
                //20170619 add by zhangcn for P001-170606002 ===beigin===
